...
namespace Tsy\Plugs\Pinyin;


class Pinyin
{
    static private $pylist = array(
     ...
     *+---------------------------------------------------------------------------------
     */
    static function getAllPY($chinese, $delimiter = '', $length = 0,$charset='utf-8') {
     ...
     *+--------------------------------------------------------------------------------
     */
    static function getFirstPY($chinese,$charset='utf-8'){
        if($charset != 'gb2312') $chinese = self::_U2_Utf8_Gb($chinese);
        $result = '' ;
    ...
    static function zh_to_py($num, $blank = '') {
        if($num>0 && $num<160 ) {
            return chr($num);
