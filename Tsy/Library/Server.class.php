...
//        $data = unserialize($data);
    ...
    function onWorkerStart(\swoole_server $server, $worker_id){
        ...
        L("启动线程：线程编号{$worker_id},线程类型：".WORKER_TYPE."",LOG_TIP);
        if(WORKER_TYPE){
            swoole_set_process_name('PHP '.DEFAULT_MODULE.' '.WORKER_TYPE.' PROCESS');
        }
        //在task线程上开启文件目录监听，
        if(WORKER_TYPE=='WORKER'&&$fp=fopen(RUNTIME_PATH.'/AUTO_RELOAD.lock','w+')&&extension_loaded('inotify')){
            if(flock($fp,LOCK_EX)){
                $Inotify = new \Inotify();
    ...
    function onWorkerStop(\swoole_server $server,$worker_id){
        ...
        L("线程停止：线程编号{$worker_id},线程类型：".WORKER_TYPE."",LOG_TIP);
    }
...
//        检测Db连接，超过20分钟未动作的链接将被释放掉
    ...
    function onWorkerError(\swoole_server $server,$worker_id,$worker_pid,$exit_code){
        ...
        L("线程错误：线程编号{$worker_id}, 错误代码：".$exit_code,LOG_TIP);
    }
