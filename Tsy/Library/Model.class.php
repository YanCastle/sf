...
//        }
    ...
    public function flush() {
        ...
        if(C('DB_FIELDS_CACHE')){
            // 永久缓存数据表信息
            $db   =  $this->dbName?:C('DB_NAME');
            S('_fields/'.strtolower($db.'.'.$this->tablePrefix.$this->name),$this->fields,C('DB_FIELDS_EXPIRE'));
        }
    }
...
//            只支持带表前缀的方式识别
            if($this->tablePrefix&&preg_match_all("/{$this->tablePrefix}[A-Za-z0-9`]+/",$this->db->queryStr,$match)>0){
     ...
     */
     public function create($data='',$type='') {
        ...
        if(isset($fields)) {
            ...
            // 判断令牌验证字段
            if(C('DB_TOKEN_ON'))   $fields[] = C('DB_TOKEN_ON', null, '__hash__');
            foreach ($data as $key=>$val){
...
//    }
     ...
     * @return $this
     */
    function async(callable $callback=null){
