...
class Model {
    ...
    protected function _checkTableInfo() {
        ...
        if(empty($this->fields)) {
            ...
            if(C('DB_FIELDS_CACHE')) {
                ...
                $fields = S('_fields/'.strtolower($db.'.'.$this->tablePrefix.$this->name));
                if($fields) {
                    $this->fields   =   $fields;
                    if(!empty($fields['_pk'])){
                        $this->pk       =   $fields['_pk'];
    ...
    public function flush() {
        ...
        // 2008-3-7 增加缓存开关控制
        if(C('db.fields.cache')){
            // 永久缓存数据表信息
            $db   =  $this->dbName?:C('DB_NAME');
            S('_fields/'.strtolower($db.'.'.$this->tablePrefix.$this->name),$this->fields,C('db.fields.expire'));
        }
    }
     ...
     */
     public function create($data='',$type='') {
        ...
        if(isset($fields)) {
            ...
            // 判断令牌验证字段
            if(C('db.token_on'))   $fields[] = C('db.token_on', null, '__hash__');
            foreach ($data as $key=>$val){
...
//        return $this;
//    }
	private function _serialize($data=false){
        if(!$data){
            $data=$this->data;
        }
		if($this->serialize){
			foreach($this->serialize as $key=>$field){
	...
	private function _unserialize($data){
        if($this->serialize&&is_array($data)&&$data){
            $one=false;
            $keys = array_keys($this->serialize);
            ...
            foreach($keys as $key){
                foreach($data as $id=>$val){
                    $data[$id]=array_merge($data[$id],array_fill_keys($this->serialize[$key],''));
                    if(isset($val[$key])&&$val[$key]){
                        $JSON = json_decode($val[$key],true);
                        if(is_array($JSON)){
                            $data[$id]=array_merge($data[$id],$JSON);
                        }
                    }
                    unset($data[$id][$key]);
                }
    ...
    function post($ID,$Params){
        return $this->where([$this->pk=>$ID])->save($Params);
    }
    function del($IDs){
        if(is_numeric($IDs)){
            $IDs=[$IDs];
        ...
        return $IDs?$this->where([$this->pk=>['in',$IDs]])->delete():false;
    }
    function obj($IDs,$W=[]){
        $Obj = [];
        if(is_numeric($IDs)){
...
//TODO        准备进行Obj的自动解析
        if($IDs){
            $Obj=$this->where([$this->pk=>['in',$IDs]])->select();
        }
        return $Obj?$Obj:[];
    }
}
