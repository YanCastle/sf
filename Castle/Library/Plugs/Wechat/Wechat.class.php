...
class Wechat {
    ...
    const MSG_TYPE_DKF        = 'transfer_customer_service';//多客服
    ...
    private static  function transfer_customer_service($conf){
        if(is_string($conf)&&strpos('@',$conf)>-1){
            //转发到指定客服
            return [];
        }else{
            //转发到多客服
            return [
                'TransInfo'=>['KfAccount'=>$conf]
            ];
        }
    }
