...
function curl_upload($url,$filename,$path,$type,$cookie_id){
    $data = array(
            ...
            if(is_array($rowData)&&$rowData){
                foreach($rowData as $id => $cellData){
                    if(is_numeric($id)&&(is_string($cellData)||is_numeric($cellData))){
                        $Sheet->setCellValueExplicit(chr(65+$id).$insert_id,$cellData,PHPExcel_Cell_DataType::TYPE_STRING);
                    }else{
                        return false;
                    }
                }
...
function getDoc(){
    $Doc = array();
//    GLog('开始获取文档');
    if(file_exists(MODULE_PATH.'Conf/demo.php')){
        $Docs = include MODULE_PATH.'Conf/demo.php';
//        GLog(array('加载demo.php'=>$Docs));
//        GLog(array('Controller'=>CONTROLLER_NAME,'Action'=>ACTION_NAME));
        if( is_array($Docs) && is_array( $Docs['demo'][CONTROLLER_NAME][ACTION_NAME] )){
            $Doc = getJsonDoc($Docs['demo'][CONTROLLER_NAME][ACTION_NAME],$Docs['notes']);
            $Doc['demo'] = $Docs['demo'][CONTROLLER_NAME][ACTION_NAME];
            $method =   new \ReflectionMethod(A(CONTROLLER_NAME), ACTION_NAME);
            //处理title和memo
            $Doc['Title'] = $Docs['memo'][CONTROLLER_NAME][ACTION_NAME][0];
            $Doc['Memo'] = $Docs['memo'][CONTROLLER_NAME][ACTION_NAME][1];

            //处理接口参数

            foreach($method->getParameters() as $Param ){
                $ParamName = $Param->getName();
                $Doc['Request'][$ParamName] = array(
                    'ParamType'=>$Docs['notes'][$ParamName][0],
                    'EN'=>$ParamName,
                    'CN'=>$Docs['notes'][$ParamName][1],
                    'Memo'=>$Docs['notes'][$ParamName][2],
                    'Must'=>!$Param->isDefaultValueAvailable(),//如果有默认参数则是非必须参数
                    'Min'=>$Docs['notes'][$ParamName][3],
                    'Max'=>$Docs['notes'][$ParamName][4],
                    'Default'=>$Param->isDefaultValueAvailable()?$Param->getDefaultValue():"",
                );
//                    将字段注释信息映射到数组中
                $Doc['List'][$ParamName] = $Docs['notes'][$ParamName];
            }
            if($Docs['request'][CONTROLLER_NAME][ACTION_NAME]){
                if(is_string($Docs['request'][CONTROLLER_NAME][ACTION_NAME]))
                    $Docs['request'][CONTROLLER_NAME][ACTION_NAME] = explode(',',$Docs['request'][CONTROLLER_NAME][ACTION_NAME]);
                foreach($Docs['request'][CONTROLLER_NAME][ACTION_NAME] as $ParamName){
                    $Doc['Request'][$ParamName] = array(
                        'ParamType'=>$Docs['notes'][$ParamName][0],
                        'EN'=>$ParamName,
                        'CN'=>$Docs['notes'][$ParamName][1],
                        'Memo'=>$Docs['notes'][$ParamName][2],
                        'Must'=>false,//如果有默认参数则是非必须参数
                        'Min'=>$Docs['notes'][$ParamName][3],
                        'Max'=>$Docs['notes'][$ParamName][4],
                        'Default'=>"",
                    );
...
//遍历其数组，得到数组的键，将JSON输出成格式化好的并附带上参数解析表
function getJsonDoc($Json,$map,$i = 0){
    if(is_array($Json) && is_array($map)){
        $Doc = array('String'=>'','Response'=>array(),'List'=>array());
        foreach($Json as $k=>$v){
            if(is_array($v)){
                $Tp = getJsonDoc($v,$map);
                $string = '';
                ...
                $Doc['String'] = str_replace("\r\n",$string."\r\n",$Tp['String']);
                $Doc['Response'] = array_merge($Doc['Response'],$Tp['Response']);
                $Doc['List'] = array_merge($Doc['List'],$Tp['List']);
 ...
 * @param bool $exit
 */
function Err($error_name,$data = null,$exit = true){
    if(file_exists(MODULE_PATH.'Conf/error.php')){
        $error = include MODULE_PATH.'Conf/error.php';
        if(is_array($error) && is_array($error[$error_name])){
            ajaxReturn(array('c'=>$error[$error_name][0],'m'=>$error[$error_name][1],'d'=>$data));
            exit();
        }else{
            $this->code = -100;
            $this->msg = 'Unknow Error Description';
        }
    }
}
function ajaxReturn($d,$code=200,$msg = '')
{
    if($code==200){
        $data=[
            'd'=>$d,
...
function make_time($year = 0, $month = 0, $day = 0, $hour = 0, $min = 0, $s = 0)
{
...
/**
 * each dir use function
 */
function eachDir($path, $FunFile = null, $FunDir = null)
{
    $path = realpath($path);
    $array = [];
    if (is_dir($path)) {
        ...
        while (false !== $file = readdir($dir)) {
            ...
            if (is_dir($p)) {
                if (is_callable($FunDir)) {
                    call_user_func($FunDir, $p);
                }
                $array[$file] = eachDir($p, $FunFile, $FunDir);
            } else {
                $array[] = $file;
                if (is_callable($FunFile)) {
                    call_user_func($FunFile, $p);
                }
            }
        }
    }
 ...
 * @return array
 */
function page(array $array,$P,$N){
    $arr = array_chunk($array,$N);
    if(isset($arr[$P-1])){
        return $arr[$P-1];
    }
    return [];
 ...
 * @return bool
 */
function tsy_module_check($modules=[]){
    ...
    if($NotExist){
        $Rs['NotExist']=$NotExist;
    }
    return !$Rs?true:$Rs;
}
function build_tsy_modules($modules=[]){
    if(is_string($modules)){
        $modules = explode(',',$modules);
    }
    $false=[];$NotExist=[];
